...
**today**, at Phase 5: what each autonomy mode actually does, how the kill
...
| Panel | Shows |
| --- | --- |
| **Market Watch** | Live ticks for watched instruments (venue clock). |
| **Signal Feed** | Incoming signals (price alerts, news) per the watchlist. |
| **Thesis Feed** | AI theses, with invalidations struck through. |
| **Decision Queue** | Proposed/approved decisions; in Assisted mode this is where you approve or reject. |
| **Watchlist** | The instruments the pipeline acts on; add/remove here. |
| **Portfolio** | Open positions, cash, and P&L (paper book). |
| **Calibration** | ECE and the Appendix B reliability gates — how well-calibrated the AI's confidence has been. |

Panels rehydrate on load by replaying recent event history, so a refresh never
loses state.
...
Phase 6, real) is committed. The AI cannot bypass it. A proposal is **rejected**
...
## 7. What is *not* here yet (Phase 6 boundary)
