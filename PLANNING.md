...
> **Status:** v0.5 — Phases 0–5 implemented (see README for current state); pre-Phase-6 hardening in progress; Phase 6+ pending. Roadmap re-scoped 2026-06-10: Phase 5 is dynamic watchlist + multi-instrument scale (instrument-agnostic feed routing, equity stub); former Phases 5/6/7 are now 6/7/8. The live-trading gating work — kill-switch coverage, single-source-of-truth autonomy mode, portfolio rehydration, ledger-accounting fixes, idempotency keys, LLM output validation — is tracked in `docs/pre-phase-6-issues.md`.
...
*End of v0.5. All six §12 decisions are LOCKED. Phases 0–5 complete; pre-Phase-6 hardening in progress (`docs/pre-phase-6-issues.md`). Revise as Phase exits reveal new constraints (and re-confirm the live execution venue before Phase 6 — market-data sourcing is settled: Kraken primary, Coinbase secondary).*
