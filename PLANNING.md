...
### Phase 6 — Alternative Data Source Integration
...
- **Phase 6B — Auto-discovery.** Promote a high-conviction alt-data signal on an *unwatched* instrument to **auto-add** it to the watchlist (→ `FeedRouter` → `EquityFeed` price → tradable), behind guardrails: a discovery cap, operator confirm and/or auto-expiry, and **liquidity-aware sizing** for the illiquid small-caps these signals surface (the current risk engine is liquidity-blind). Deliberately split out of 6A — discovery is a control-plane problem, not an ingestion one — and kept out of the next code pass, but on record so it is not lost.
  - *Exit:* a qualifying disclosure on an unwatched name can, within caps, bring that name into the watched / price-fed / tradable set automatically, with bounded watchlist growth and liquidity-aware sizing. **Still paper.**

### Phase 7 — Live Trading (Assisted), in four graduated sub-phases

> *Renumbered from Phase 6 by ADR-010 (2026-06-13); the new Phase 6 is alternative-data ingestion. Sub-phases 6A–6D below are now 7A–7D. Historical companions keep their filenames — `docs/phase-6-plan.md`, `docs/pre-phase-6-issues.md` — read one phase number higher.*
...
*End of v0.7. All six §12 decisions are LOCKED — the live execution venue is settled (Alpaca primary + Kraken secondary, ADR-009; market-data sourcing unchanged: Kraken primary, Coinbase secondary). Phases 0–5 complete. **Phase 6 (alternative-data ingestion, ADR-010) is the next code pass — 6A signal feeds (enrich-only) then 6B auto-discovery.** Live trading is now Phase 7, staged 7A–7D (`docs/phase-6-plan.md`); its pre-live-trading hardening is complete bar the gateway auth/bind entry gate + hygiene cleanups (`docs/pre-phase-6-issues.md`). Revise as Phase exits reveal new constraints.*
