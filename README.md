...
- [Plain-language overview](docs/pipeline-simple.svg) - the same flow, no jargon.
...
Each top-level directory is a Python package sharing one `pyproject.toml`; everything depends on
`core/`, which depends on nothing. The frontend is a separate Vite app.

| Path | Responsibility |
|---|---|
| `core/` | Shared domain - schemas, event bus, SQLite, `ModeController`, pricing |
| `watchlist/` | User-managed instrument registry (`WatchlistManager` + store protocol) |
| `ingestion/` | Market-data feeds (Kraken, equity stub, Coinbase) + signal generators (alerts, news) + alt-data pollers (insider, gov-exposure, supply-chain, congress) + `FeedRouter`/`TickPruner` |
| `reasoning/` | LLM layer - provider abstraction, `ThesisGenerator`, `DecisionGenerator` |
...
| `analytics/` | Risk/return metrics + equity-curve projection (the economic gate's read side) |
| `discovery/` | Multi-source candidate surfacing - confluence scoring + AI analyst (6B.1) |
| `backtest/` | Event-time replay engine + CLI (`python -m backtest`) |
| `gateway/` | FastAPI app - REST routes + WebSocket broadcaster |
| `frontend/` | React + Vite terminal UI |
| `docs/` | Documentation - start at [`docs/README.md`](docs/README.md) |

The component-by-component breakdown, data-flow traces, and the event-bus contract live in
...
**Autonomy is graduated.** Five modes - Observe → Paper → Assisted → Semi-auto → Supervised - with explicit promotion criteria and automatic demotion triggers. The first three are operable today; Semi-auto/Supervised arrive with live trading (Phase 7+). Kill switch available at all times.
...
| **6B.1** | Discovery engine (disclosure-driven) | Live: `discovery/` confluence scoring (noisy-OR over 6A factor tags, decay) as a pull-first projection → `GET /api/discovery` + per-candidate AI analyst; terminal Discover workspace with ranked feed, evidence, one-click watchlist-add. Equity, manual promotion (ADR-012) |
| **6B.2** | Discovery breadth scanner + control plane | Broad-universe scanner (Alpaca screener / CoinGecko / Kraken listings) → crypto-primary; auto-add with cap + TTL/cooldown (the liquidity floor + ADV-% size cap are pulled forward to Phase 7 - see [`docs/pre-phase-7-risk-review.md`](docs/pre-phase-7-risk-review.md) §11) |
